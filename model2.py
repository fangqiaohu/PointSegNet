...
#         self.fc3 = nn.Linear(256, num_paras)
#
#     def forward(self, xyz):
#         B, _, _ = xyz.shape
#         l1_xyz, l1_points = self.sa1(xyz, None)
...
if __name__ == '__main__':
    ...
    N_pts = 1024
    N_channel = 3
    sim_in = torch.randn((batchsize, N_channel, N_pts))
    print(sim_in.size())

    # pointnet2semseg = PointNet2SemSeg(num_classes=2)
    # sim_out = pointnet2semseg(sim_in)
    # print(sim_out.size())

    modelreg = ModelReg(num_paras=6)
    sim_out = modelreg(sim_in)
    print(sim_out.size())
